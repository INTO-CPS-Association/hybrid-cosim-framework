...
namespace adaptation 
{
	...
	{			
		...
		this->internalState.current_psu = 0.0;
		...
		this->internalState.previous_psu = 0.0;
	...
	void PlantMultiRate::in_rule_body1(double dt, double H, double h){
		...
		this->internalState.current_psu = this->internalState.psu;
	...
	void PlantMultiRate::in_rule_flush1(double dt, double H, double h){
		...
		;
		setValue(plant,PLANTTORQUE,this->internalState.previous_torque + (((this->internalState.current_torque - this->internalState.previous_torque) / H) * dt));
		;
		setValue(plant,PLANTPSU,this->internalState.previous_psu + (((this->internalState.current_psu - this->internalState.previous_psu) / H) * dt));
		;
		setValue(plant,PLANTX,this->internalState.previous_x + (((this->internalState.current_x - this->internalState.previous_x) / H) * dt));
		;
		setValue(plant,PLANTV,this->internalState.previous_v + (((this->internalState.current_v - this->internalState.previous_v) / H) * dt));
		;
	}
	...
	double PlantMultiRate::executeInternalControlFlow(double t, double H) {
		...
		this->internalState.previous_psu = this->internalState.current_psu;
