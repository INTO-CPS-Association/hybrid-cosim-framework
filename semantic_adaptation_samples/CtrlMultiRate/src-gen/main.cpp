...
int main(int argc, char *argv[]) {
	...
	fmi2Component comp =
			fmi2Instantiate("this system", fmi2CoSimulation, "{1234}",
					"D:\\srcctrl\\into-cps-organization\\hybridCosimulation-framework\\semantic_adaptation_samples\\CtrlMultiRate",
					&callback, fmi2True,
					fmi2True);
...
#define ID_OUT 3
	...
	for (double time = 0.0; time < 2*stepSize; time += stepSize) {
		...
		if (setReal(comp, ID_V_IN, time) != fmi2OK) {
			printf("Error setReal");
			return 1;
		}
		if (setReal(comp, ID_X_IN, time) != fmi2OK) {
			printf("Error setReal");
			return 1;
		}
		if (setReal(comp, ID_REF_IN, time + stepSize) != fmi2OK) {
			printf("Error setReal");
			return 1;
		}
		...
		auto out = getReal(comp, ID_OUT);

		cout << "time: " << time + stepSize << " out: " << out
				<< endl;
	}
